...
class selftest_test_SnmpHelper(rootfs_boot.RootFSBootTest):
    ...
    def runTest(self):
        ...
        unit_test = SnmpMibsUnitTest(mibs_location = './resources/mibs',
                                     files = ['SNMPv2-MIB'],
                                     mibs = test_mibs,
                                     err_mibs = wrong_mibs)
...
class selftest_test_cmts(rootfs_boot.RootFSBootTest):
   ...
   def runTest(self):
       ...
       cmts_status = cmts.check_online(board.config['cm_mac'])
       print "Obtained the status of the cmts: %s" % cmts_status
