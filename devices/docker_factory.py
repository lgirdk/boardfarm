...
class DockerFactory(linux.LinuxDevice):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.expect_exact('export PS1="docker_session>"')
        self.expect(self.prompt)
        self.set_cli_size(200)

        # iface set, we need to create network
        if self.iface is not None:
            self.sendline('docker network create -d macvlan -o parent=%s -o macvlan_mode=bridge %s' % (self.iface, self.iface))
            self.expect(self.prompt)
        ...
        self.sendline('docker run --rm --name=%s -d -p 22 %s /usr/sbin/sshd -D' % (self.iface, self.target_img))
        self.expect(self.prompt)
        self.expect(pexpect.TIMEOUT, timeout=1)
        self.sendline('docker network connect %s %s' % (self.iface, self.iface))
        self.expect(self.prompt)
        self.sendline('docker exec %s ip address flush dev eth1' % self.iface)
        self.expect(self.prompt)
        self.sendline("docker port %s | grep '22/tcp' | sed 's/.*://g'" % self.iface)
        self.expect_exact("docker port %s | grep '22/tcp' | sed 's/.*://g'" % self.iface)
        self.expect(self.prompt)
    ...
    def clean_docker_network(self):
        if self.created_docker_network == True:
            self.sendline('docker network rm %s' % self.iface)
            self.expect(self.prompt)
            self.sendline('docker network ls')
            self.expect(self.prompt)
    ...
    def clean_docker(self):
        if self.created_docker == True:
            self.sendline('docker stop %s' % self.iface)
            self.expect(self.prompt)
            self.sendline('docker rm %s'% self.iface)
            self.expect(self.prompt)
