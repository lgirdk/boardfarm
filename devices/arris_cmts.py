...
class ArrisCMTS(base_cmts.BaseCmts):
    ...
    def check_online(self, cmmac):
        ...
        # makes it easier on us lets get just the one line of interest
    ...
    def clear_cm_reset(self, cmmac):s
...
if __name__ == '__main__':
    ...
    cmts = ArrisCMTS(conn_cmd=sys.argv[1], connection_type=connection_type)
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "check_online":
        cm_mac = sys.argv[3]
        status = cmts.check_online(cm_mac)
        print "Status for "+cm_mac+"="+str(status)
        sys.exit(0)
