...
class DockerFactory(DebianBox):
    ...
    def spawn_containers(self, count, _iface = None):
        """
        This method is invoked by the test case
        count:      the number of containers to be created per interface
        interfaces: an override for the interface value in the json
        ...
        # must be a list not a string, or else all  containers might be deleted!!!!
        assert not isinstance(self.interfaces, basestring)
        ...
        for iface_idx,iface_and_vlan in enumerate(self.interfaces):
            ...
            for c in range(self.count):
                ...
                if vlan is None:
                    ...
                    _name = self.dyn_dev_name + index

                try:
                    klass = globals()[self.container_type]

                    print("Creating Session for: name=" + _name + ", ipaddr=" + self.ipaddr + ", port=" + str(p) + ", color=" + self.color)
