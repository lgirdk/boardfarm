    ...
    def get_interface_ipv4addr(self, value):
        ...
        if ipaddr:
            ...
            try:
                ...
                raise BFTypeError("Failed at calculating IP address for the given data - device: %s interface: %s"%(self.dev.name,self.iface))
    ...
    def get_interface_ipv6addr(self, output):
        ...
        if ipv6_list:
            for var in ipv6_list:
                ipaddr = re.search('inet6\s(([0-9a-fA-F]{0,4}[:]{0,2}[/\d]*)*).*',var)
                try:
                    if 'global' in var:
                        self.global_ipv6_list = []
                        self.global_ipv6_list.append(ipaddress.IPv6Interface(six.text_type(ipaddr.group(1))))
                        self._ipv6 = self.global_ipv6_list[0]
                    if 'link' in var:
                        self._ipv6_link_local = ipaddress.IPv6Interface(six.text_type(ipaddr.group(1)))
