...
def clean_for_json(data):
    """
    Given a python dictionary walk the structure and convert values to types that are valid for JSON.
...
def get_test_name(test, override_name=None):
    """Given a test, return a test name."""
...
def generate_test_info_for_kibana(test, prefix="", override_name=None):
    """Generate the test info for kibana.
...
def check_devices(devices, func_name="check_status"):
    ...
    For each device, run a function. Useful to see if devices are still
