...
class Set_IPv6_Addresses(rootfs_boot.RootFSBootTest):
    ...
    def runTest(self):
        ...
        # Lan-side Device
        lan.sendline(f"\nip -6 addr add 4aaa::6/64 dev {lan.iface_dut}")
        lan.expect("ip -6")
        lan.expect(prompt)
        lan.sendline(f"ip -6 route add 4aaa::1 dev {lan.iface_dut}")
        lan.expect(prompt)
        lan.sendline(f"ip -6 route add default via 4aaa::1 dev {lan.iface_dut}")
        lan.expect(prompt)
        if "No route to host" in lan.before:
            raise Exception("Error setting ivp6 routes")
        # Wan-side Device
        wan.sendline(f"\nip -6 addr add 5aaa::6/64 dev {wan.iface_dut}")
        wan.expect("ip -6")
        wan.expect(prompt)
        wan.sendline(f"ip -6 route add 5aaa::1 dev {wan.iface_dut}")
        wan.expect(prompt)
        wan.sendline(f"ip -6 route add default via 5aaa::1 dev {wan.iface_dut}")
        wan.expect(prompt)
